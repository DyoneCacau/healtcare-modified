-- ============================================================
-- PRODUÇÃO 40 — SMART HUB: PAYLOAD PÚBLICO RESTRITO
-- ============================================================
-- INSTRUÇÕES:
-- 1. Pré-requisitos: Smart Hub base (PRODUCAO_29+), PRODUCAO_36
--    (coluna public_booking_enabled).
-- 2. Revise este arquivo antes de executar.
-- 3. No Supabase: SQL Editor > New query.
-- 4. Cole TODO o conteúdo e clique em Run.
-- 5. Script IDEMPOTENTE (CREATE OR REPLACE).
-- 6. Este script NÃO sobe com o deploy da Vercel — execução manual.
-- 7. Depois: NOTIFY pgrst, 'reload schema';
--
-- O que faz:
-- - Restringe get_public_smart_hub a campos necessários na página pública
-- - Remove clinic_id, donos CRM, storage_path e metadados internos do anon
-- - Sanitiza capture_config (hub e botões) sem owner / etapa / dedupe
-- ============================================================

BEGIN;

-- ---------------------------------------------------------------------------
-- Helpers: sanitização de capture_config para resposta pública
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.smart_hub_public_capture_config(jsonb) IS
  'Subset público de capture_config do Hub (sem owner/etapa/dedupe).';
COMMENT ON FUNCTION public.smart_hub_public_button_capture_config(jsonb) IS
  'Subset público de capture_config do botão (sem owner/etapa).';

-- ---------------------------------------------------------------------------
-- RPC pública: whitelist explícita (não usa to_jsonb da linha inteira)
-- ---------------------------------------------------------------------------
